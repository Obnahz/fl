...
try {
  ...
  for (let attempt = 0; attempt < 12 && !(await page.locator('.combat-result').count()); attempt++) {
    await exploreButton.click()
    await page.waitForTimeout(250)
  }

  await page.locator('.combat-result').waitFor()
  await page.getByText('最近战斗', { exact: true }).waitFor()
  await page.getByText('雾尾灵鼠', { exact: true }).first().waitFor()

  const desktopPath = 'C:/Users/z1993/Documents/New project 2/vue-idle-xiuxian-main/tests/phase5-desktop.png'
  await page.screenshot({ path: desktopPath, fullPage: true })

  await page.setViewportSize({ width: 390, height: 844 })
  await page.waitForTimeout(200)
  const mobileOverflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth)
  const mobilePath = 'C:/Users/z1993/Documents/New project 2/vue-idle-xiuxian-main/tests/phase5-mobile.png'
  ...
  const result = {
    ...
    mobilePath,
    mobileOverflow,
    outcome: await page.locator('.combat-result').innerText(),
    errors
  }
