...
try {
  ...
  await page.evaluate(async () => {
    ...
    store.techniqueLevels = { spirit_edge: 1, thunder_sword_intent: 1 }
    store.techniqueFragments = { thunder_sword_intent: 2 }
  })

  await page.getByText('功法', { exact: true }).first().click()
  await page.getByRole('heading', { name: '功法' }).waitFor()
  await page.getByText('惊雷剑意', { exact: true }).waitFor()

  const thunderCard = page.locator('.technique-card').filter({ hasText: '惊雷剑意' })
  await thunderCard.getByRole('button', { name: '设为出战' }).click()
  await thunderCard.getByText('当前出战', { exact: true }).waitFor()
  await thunderCard.getByRole('button', { name: '参悟升级' }).click()
  await thunderCard.getByText('第 2 / 5 层', { exact: true }).waitFor()
  await thunderCard.getByText('1.62x', { exact: true }).waitFor()

  const desktopPath = 'C:/Users/z1993/Documents/New project 2/vue-idle-xiuxian-main/tests/phase7-desktop.png'
  await page.screenshot({ path: desktopPath, fullPage: true })

  await page.setViewportSize({ width: 390, height: 844 })
  await page.waitForTimeout(200)
  const mobileOverflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth)
  const mobilePath = 'C:/Users/z1993/Documents/New project 2/vue-idle-xiuxian-main/tests/phase7-mobile.png'
  await page.screenshot({ path: mobilePath, fullPage: true })

  await page.getByText('历练', { exact: true }).first().click()
  await page.getByRole('heading', { name: '历练' }).first().waitFor()
  await page.getByText('惊雷剑意 · 第2层', { exact: true }).waitFor()
  await page.evaluate(async () => {
    const { usePlayerStore } = await import('/src/stores/player.js')
  ...
  await page.getByText(/施展惊雷剑意/).first().waitFor()
  ...
  console.log(
    JSON.stringify({
      ...
      mobilePath,
      mobileOverflow,
      activeTechnique: await page.locator('.technique-panel').innerText(),
