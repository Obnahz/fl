    ...
    <n-card title="游戏设置">
      ...
      <n-space vertical>
        ...
        <n-space>
          ...
          <n-button target="_blank" href="https://github.com/Obnahz/fl" tag="a" type="primary">
            开源地址
          </n-button>
          <n-button type="error" @click="qq = true">联系我们</n-button>
        </n-space>
      </n-space>
    </n-card>
    <n-modal preset="dialog" title="官方邮箱" v-model:show="qq">
      <n-card :bordered="false" size="huge" role="dialog" aria-modal="true">
        <n-space vertical>
          <n-input value="1993593799@qq.com" readonly type="text" />
        </n-space>
      </n-card>
    </n-modal>
  </div>
...
<script setup>
  ...
  const handleImportSave = data => {
    ...
    reader.onerror = () => message.error('读取存档文件失败。')
    reader.onabort = () => message.warning('已取消导入存档。')
