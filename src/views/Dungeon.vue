...
<script setup>
  ...
  const createPlayerEntity = () => {
    ...
    const linked = buildDungeonPlayerCombatant({
      player: {
        ...
        activeEquipmentSetBonuses: playerStore.activeEquipmentSetBonuses,
        getPetBonus: playerStore.getPetBonus,
        baseIncludesPet: Boolean(playerStore.activePet),
        activeEffects: playerStore.activeEffects
      },
      technique
    })
    ...
    dungeonBuffs.getActiveBuffs().forEach(buff => {
      if (typeof buff.effect === 'function') buff.effect(entity)
    })
    /*
    // 基础属性
    const baseStats = {
  ...
  const autoCombat = async () => {
    while (dungeonState.value.inCombat) {
      ...
      // 添加动画效果
      const latestAction = result.results?.at(-1)
      if (latestAction?.attacker === dungeonState.value.combatManager.player.name) {
      ...
      if (result.state === 'victory') {
        playerStore.currentHealth = Math.max(0, Math.round(dungeonState.value.combatManager.player.currentHealth))
        handleVictory()
        break
...
<style scoped>
  ...
  .option-card:hover {
    ...
    box-shadow: 0 10px 24px rgba(16, 39, 25, 0.12);
  }
  ...
  .combat-scene {
    ...
    margin-bottom: 20px;
    min-height: 200px;
    background: color-mix(in srgb, var(--jade-pale) 28%, var(--surface));
    border: 1px solid var(--line);
    border-radius: 6px;
  }
  ...
  .health-bar {
    width: 100px;
    height: 10px;
    background: rgba(180, 77, 60, 0.16);
    border-radius: 3px;
    overflow: hidden;
  }
  ...
  .player-avatar {
    background: var(--jade);
    border: 1px solid rgba(18, 59, 49, 0.4);
    border-radius: 6px;
  }

  .enemy-avatar {
    background: var(--cinnabar);
    border: 1px solid rgba(122, 43, 35, 0.4);
