...
class CombatStats {
  ...
  // 计算伤害减免
  calculateDamageReduction(incomingDamage, attackerStats, armorPenetration = 0) {
    let damage = Math.abs(incomingDamage)
    // 应用防御减伤（考虑战斗属性提升）
    const effectiveDefense = this.defense * (1 + this.combatBoost) * (1 - Math.min(0.75, Math.max(0, armorPenetration)))
...
class CombatEntity {
  ...
  takeDamage(amount, source, context = {}) {
    ...
    // 计算实际伤害
    const reducedDamage = this.stats.calculateDamageReduction(amount, source?.stats, context.armorPenetration)
...
class CombatManager {
  ...
  executeTurn() {
    ...
    // 第一回合攻击
    const firstTechnique = firstAttacker === this.player && this.player.technique && this.round >= this.nextTechniqueRound
      ? this.player.technique
      : null
    const firstAttack = firstAttacker.stats.calculateDamage(secondAttacker, firstTechnique || {})
    const firstResult = secondAttacker.takeDamage(firstAttack.damage, firstAttacker, firstTechnique || {})
    if (firstTechnique) this.nextTechniqueRound = this.round + Math.max(1, firstTechnique.cooldownRounds || 1) + 1
